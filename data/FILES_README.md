...
| `touched_lastwinner` | `yes` = sent a tx to the contract (full block-range scan, 63,456 unique callers, converged through mid-Sep 2018) OR holds a pID/vault. `unknown` = not confirmed in that data (means "not found", not "confirmed absent"). **35,527 `yes` (80.7%)**, 8,487 `unknown` (19.3%). The `unknown` set is almost all loose-ETH-only (7,654 addrs). A 220-address full-history pull (Alchemy, no scan-window cutoff) finds only ~1% of that tight set ever touched the game contract and 0% touched FoMo3D — i.e. it really is a non-playing subset, NOT (as an earlier note here said) mostly 2018 players who withdrew early. It is still part of the same operation: 88% funded in the two 2018 fleet waves, ~46% used the same coToken / batch-distributor cluster as confirmed players (~52%), and 100% swept in 2026 to the same collectors — the pooled-betting + reserve-wallet wing. See `summary.json` → `TASK_characterise_the_non_lastwinner_subset`. |
